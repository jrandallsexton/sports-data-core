...
import InsightDialog from "../insights/InsightDialog.js"; // Import the dialog component
import mockInsight from "../../data/insights"; // Adjust the path if needed

function PicksPage() {
  const [userPicks, setUserPicks] = useState({});
  const [submitted, setSubmitted] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isSubscribed, setIsSubscribed] = useState(false); // Hardcoded false for now
  ...
  function handleViewInsight(matchup) {
    setSelectedMatchup({
      ...matchup,
      insightText: "", // Clear old text initially
    });
    ...
    setTimeout(() => {
      setSelectedMatchup((prev) => ({
        ...prev,
        bullets: mockInsight.bullets, // Always use the mocked insight for now
        prediction: mockInsight.prediction, // Always use the mocked prediction for now
      }));
