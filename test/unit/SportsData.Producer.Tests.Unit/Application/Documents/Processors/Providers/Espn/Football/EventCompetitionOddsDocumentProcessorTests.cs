...
namespace SportsData.Producer.Tests.Unit.Application.Documents.Processors.Providers.Espn.Football
{
        ...
        : ProducerTestBase<EventCompetitionOddsDocumentProcessor<FootballDataContext>>
    {
        ...
        [Fact]
        public async Task Processor_Persists_PointSpread_FromPointSpreadBlocks_SingleDto_Alt()
        {
            // Arrange: seed contest/competition
            var compId = Guid.NewGuid();
            var contest = Fixture.Build<Contest>()
            ...
            // load the SINGLE-DTO payload you pasted
            var json = await LoadJsonTestData("EspnFootballNcaaEventCompetitionOdds_VandyBama_02Oct25.json");
            ...
            // These MUST match the JSON pointSpread.* (lines), not spread.* (prices)
            away.SpreadPointsOpen.Should().Be(11.5m);
            home.SpreadPointsOpen.Should().Be(-11.5m);

            away.SpreadPointsCurrent.Should().Be(+10.5m);
            home.SpreadPointsCurrent.Should().Be(-10.5m);

            // And open prices come from spread.* blocks
            away.SpreadPriceOpen.Should().Be(-110m);
            home.SpreadPriceOpen.Should().Be(0m);
        }
    }
}
