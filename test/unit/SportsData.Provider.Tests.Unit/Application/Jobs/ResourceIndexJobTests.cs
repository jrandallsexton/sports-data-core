...
public class ResourceIndexJobTests : ProviderTestBase<ResourceIndexJob>
{
    ...
    [Fact]
    public async Task When_DocumentCountsMatch_Should_ShortCircuit()
    {
        // Arrange
        var def = Fixture.Create<DocumentJobDefinition>();

        var resourceIndex = Fixture.Build<ResourceIndex>()
            .With(x => x.Id, def.ResourceIndexId)
            .Create();

        DataContext.ResourceIndexJobs.Add(resourceIndex);
        await DataContext.SaveChangesAsync();

        var dtoJson = await LoadJsonTestData(SinglePageJson);
        var dto = JsonSerializer.Deserialize<EspnResourceIndexDto>(dtoJson);
        ...
        Mocker.GetMock<IDocumentStore>()
            ...
            .ReturnsAsync(dto!.Items.Select(i => Fixture.Build<DocumentBase>().Create()).ToList());
    ...
    [Fact]
    public async Task When_DryRun_True_Should_SkipEnqueue()
    {
        // Arrange
        var def = Fixture.Create<DocumentJobDefinition>();
        ...
        Mocker.GetMock<IDocumentStore>()
            .Setup(x => x.GetAllDocumentsAsync<DocumentBase>(It.IsAny<string>()))
            .ReturnsAsync(new List<DocumentBase>()); // force mismatch
    ...
    [Fact]
    public async Task When_DryRun_False_And_CountMismatch_Should_Enqueue_One_Job_Per_Item()
    {
        // Arrange
        var def = Fixture.Create<DocumentJobDefinition>();
