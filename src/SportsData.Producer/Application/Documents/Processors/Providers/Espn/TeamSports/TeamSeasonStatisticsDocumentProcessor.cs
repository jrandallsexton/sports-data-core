...
[DocumentProcessor(SourceDataProvider.Espn, Sport.FootballNfl, DocumentType.TeamSeasonStatistics)]
[DocumentProcessor(SourceDataProvider.Espn, Sport.BaseballMlb, DocumentType.TeamSeasonStatistics)]
    ...
    where TDataContext : TeamSportDataContext
{
    ...
    private async Task TryProcessStatistics(Guid franchiseSeasonId, ProcessDocumentCommand command, int attempt)
    {
        ...
        if (franchiseSeason.Statistics?.Any() == true)
        {
            var existingGamesPlayed = ExtractGamesPlayedFromExisting(franchiseSeason.Statistics) ?? 0;
