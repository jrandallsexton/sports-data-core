# Team Finalized Games Endpoint (Mini-Schedule v2)

Slim, snapshot-capable endpoint that backs the embedded MiniSchedule
on the PicksPage MatchupCards (web + mobile). Returns a team's
**finalized games** — games where `FinalizedUtc IS NOT NULL`, i.e.
officially closed with a result. Builds on the mini-schedule rework
shipped in PR #369.

> **Naming note.** The endpoint was briefly called "team schedule"
> during development. That was a misnomer: it never returned the full
> schedule, only the subset of finalized games. The C# method names,
> route, SQL file, hook, and DTO references all use "finalized games"
> to match the underlying schema column (`Contest.FinalizedUtc`).
> The web `MiniSchedule` component name and its `schedule` prop are
> retained — they describe the visual element, not the data semantics.
...
1. **Wrong slice for in-season teams.** The component shows the
   ...
   *finalized* games. Mid-season this means future, unplayed games
   appear at the top.
...
1. **MLB drops same-week finalized games.** A `SeasonWeek` for MLB
...
- Return **finalized games only** (`FinalizedUtc IS NOT NULL`).
...
- Slim payload: array only, no team header / record / etc.
- Same item shape as today's `TeamCardScheduleItemDto[]` so the
  existing web + mobile MiniSchedule components don't need DTO
  changes.
...
- Not introducing a new DTO shape for the items themselves.
...
```http
GET /ui/teamcard/sport/{sport}/league/{league}/team/{slug}/{year}/finalized-games
    ?asOfDate={ISO 8601, optional}
```

- Returns `TeamCardScheduleItemDto[]` directly (no envelope).
- `asOfDate` is optional. Omitted → all finalized games for the
  season.
- 200 with `[]` when the team exists but has no finalized games
...
```http
GET /api/franchises/{slug}/seasons/{year}/finalized-games
    ?asOfDate={ISO 8601, optional}
```

- Slug-routed action on `FranchisesController`, per `CLAUDE.md`
  read-endpoint convention.
- Dapper, projects to `TeamCardScheduleItemDto`, `.AsNoTracking()`
  semantics (Dapper is read-only by default).

### SQL

Embedded resource `GetTeamFinalizedGames.sql`. Same JOINs as
...
PicksPage  →  MatchupList(leagueAsOfDate)
           ...
           →  useTeamFinalizedGames(asOfDate)
           →  GET …/finalized-games?asOfDate={iso}
```

Outside a pick'em context (team page, future use cases),
`asOfDate` is omitted and the endpoint returns all finalized games
for the season.

### Hook shape

Mobile: a lightweight `useTeamFinalizedGames` hook (TanStack Query,
...
Web: the existing schedule hook was renamed to
`useTeamFinalizedGames` to hit the new endpoint instead of
stripping `schedule` out of the full TeamCard response.
...
endpoint returns everything finalized-and-eligible and the
component trims. The component name and its `schedule` prop are
retained: they describe the visual element, not the data semantics.
...
3. Core: `FranchiseClient.GetTeamFinalizedGames(slug, year, asOfDate?)`.
...
7. Mobile: same swap; new typed `useTeamFinalizedGames` hook.
...
- **Empty result for early-season teams.** A team with zero
  finalized games returns `[]`; MiniSchedule already renders
