...
- **Why does the chain hop between Ingest and Worker so many times?** Each `IConsumer<T>` runs on Ingest (its only job is to enqueue); each Hangfire job runs on Worker. Per the project's Ingest-thin-shim rule (Ingest consumers must be thin Hangfire-spawn shims; no inline DB writes on Ingest pods — pure SignalR fan-out is the only exception), this separation buys: crash-safe retry, KEDA scale-out on Hangfire queue depth, and observable backpressure. The latency cost is sub-second on warm Workers.
