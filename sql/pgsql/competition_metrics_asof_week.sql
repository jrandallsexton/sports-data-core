...
--
-- SQL-client copy of src/metrics-modeling/sql/competition_metrics_asof_week.sql
-- (that one takes psql -v variables; this one inlines them).
-- ▼▼▼ EDIT THESE THREE VALUES, THEN RUN ▼▼▼
WITH params AS (
    SELECT 2025 AS season_year,   -- season to backtest
           6    AS week,          -- the week whose slate to predict
           5    AS prior_tail     -- 0 = off; N = top up early-week windows
                                  -- with last N prior-season (reg/post) games
),
...
WHERE con."CancelledUtc" IS NULL
  AND (sp."TypeCode" IS NULL OR sp."TypeCode" <> 1)
  --AND (con."AwayTeamFranchiseSeasonId" = '3037ab67-d176-aca8-85d9-567bd30bd304' OR con."HomeTeamFranchiseSeasonId" = '3037ab67-d176-aca8-85d9-567bd30bd304')
